    ...
    def run(self):
        ...
        while True:
            try:
                ...
                #     print(f'Checking ThreadInfo.json file on HDD')

                # print(threads.THREAD_INFO)
                ...
                if self.app_is_idle is True and response.status_code == 401:
                    ...
                    #     print(f'Writing Gamedetails.json file on HDD')

                    threads.GAME_DETAILS = response

                    game_status = 1
                    print(f">>> threads - Game Status: {game_status}")
                ...
                if "401 Client Error" in str(request_error):
                    ...
                    if thread_file_response["ResettingGame"] is True:
                        ...
                        # # print(f'Writing ThreadInfo.json file to Memory')
...
class UpdateRoomInfo(QThread):
    ...
    def run(self):
        ...
        get_video_player_files_api = DEVICE_REQUEST_API.format(device_unique_code=device_unique_code)
        ...
        while True:
            try:
                ...
                # print(">>> threads.py - Checking for Room Configs")
                # response_of_room_info_api = requests.get(ROOM_INFO_API, headers=headers)
                ...
                # threads.DEVICE_CONFIG = dictionary
                ...
                json_response = requests.get(get_video_player_files_api, headers=headers)
                json_response.raise_for_status()

                deviceRequestId = json_response.json()["DeviceRequestid"]
                requestId = json_response.json()["RequestID"]
                # print(f">>> threads.py - Video Player STATUS = {requestId}")

                if requestId == 15:
                    url = POST_DEVICE_API.format(device_unique_code=device_unique_code, deviceRequestId=deviceRequestId)
                    response = requests.post(url, headers=headers)
                    if response.status_code == 200:
                        self.update_detected.emit()
                        print(f">>> threads.py - Video Player Files Updated, Reloading...")
                    else:
                        pass
...
class GetGameClue(QThread):
    ...
    def run(self):
        ...
        #     print(f'Checking Gamedetails.json file on HDD')
...
class WebsocketClientThread(threading.Thread):
    def __init__(self):
        ...
        print(">>> threads.py - WebSocket Client Thread init ....")

        # global attributes
        self.active = None
        self.WebsocketClient_instance = None

    def run(self):
        print(">>> threads.py - Starting Base WebSocket Client Thread ...")
        self.WebsocketClient_instance = websocket_client.WebsocketClient()
        print(">>> threads.py - Stopped Base WebSocket Client Thread ...")
        # websocket_client. = False
        return


#
# class WebsocketClientThread2(QThread):
#     def run(self):
#         print(">>> threads.py - Starting Base WebSocket Client Thread ...")
